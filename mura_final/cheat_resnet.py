...
target_categories = ['XR_SHOULDER',  'XR_WRIST']
batch_size = 16
num_epochs = 100
...
class CustomCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        global best_acc  # Declare best_acc as a global variable
        ...
        test_acc = test_binary_acc
        model_filename = f'{self.save_path}{self.model_name}_epoch_{epoch + 1}_testacc_{test_acc:.4f}_kappa_{test_cohen_kappa:.4f}.keras'

        if test_acc > best_acc:
            best_acc = test_acc  # Update best_acc if the current test accuracy is higher
            self.model.save(model_filename)
            print(f'\nTest accuracy after epoch {epoch + 1}: {test_acc:.4f} - Model saved as {model_filename}')
        else:
            print(f'\nTest accuracy after epoch {epoch + 1}: {test_acc:.4f}  - model not saved')
